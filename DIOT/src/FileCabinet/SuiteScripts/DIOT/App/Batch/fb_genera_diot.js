...
define([
  ...
  "N/email",
  "N/query",
  "../../Lib/Enum/fb_diot_constants_lib",
  "../../Lib/Mod/moment_diot",
], (
  newError,
  runtime,
  ...
  const getInputData = (inputContext) => {
    ...
    const recordID = objScript.getParameter({
      name: SCRIPTS_INFO.MAP_REDUCE.PARAMETERS.RECORD_DIOT_ID,
    });
    try {
      ...
      var otherId = record.submitFields({
        ...
        id: recordID,
        values: {
          [RECORD_INFO.DIOT_RECORD.FIELDS.STATUS]:
            STATUS_LIST_DIOT.OBTAINING_DATA,
          [RECORD_INFO.DIOT_RECORD.FIELDS.PROGRESS]: 0.0,
          [RECORD_INFO.DIOT_RECORD.FIELDS.FILE]: "",
        },
      });
      var oneWorldFeature = runtime.isFeatureInEffect({
        feature: RUNTIME.FEATURES.SUBSIDIARIES,
      });
      var suitetax = runtime.isFeatureInEffect({
        feature: RUNTIME.FEATURES.SUITETAX,
      });
      log.debug("Config_Instance", {
        oneWorldFeature: oneWorldFeature,
        suitetax: suitetax,
      });
      // oneWorldFeature = false;
      if (oneWorldFeature == false) {
        // si no es oneWorld o SuiteTax
        let newError = generateError(
          "ERROR DE ENTORNO",
          "Error de configuración DIOT, contacte a su administrador.",
          "Su instancia no esta configurada para trabajar con el modulo DIOT."
        );
      ...
      let registerData = search.lookupFields({
        ...
        id: recordID,
        columns: [
          RECORD_INFO.DIOT_RECORD.FIELDS.SUBSIDIARY,
          RECORD_INFO.DIOT_RECORD.FIELDS.PERIOD,
        ],
      });
      // log.audit({title: 'Estado_getInput', details: registerData});
      const recordSubsidiary =
        registerData[RECORD_INFO.DIOT_RECORD.FIELDS.SUBSIDIARY][0].value;
      const recordPeriod =
        registerData[RECORD_INFO.DIOT_RECORD.FIELDS.PERIOD][0].value;
      log.debug({
        title: "resultData",
        details: {
      ...
      let getVendorBills_result;
      {
        ...
        log.debug({
          title: "extractVendorBill_result",
          details: extractVendorBill_result,
        });
        if (extractVendorBill_result.length <= 0) {
          throw "A ocurrido un error al extraer facturas";
        }
        getVendorBills_result = getVendorBills(
          recordPeriod,
          recordSubsidiary,
          recordID,
          extractVendorBill_result
        );
        log.debug({
          title: "getVendorBills_result",
          details: getVendorBills_result,
        });
      ...
      let getExpenseReport_result;
      {
        ...
        log.debug({
          title: "extractExpenseReport_result",
          details: extractExpenseReport_result,
        });
        ...
        if (extractExpenseReport_result.data.length > 0) {
          getExpenseReport_result = getExpenseReport(
            recordSubsidiary,
            recordPeriod,
            recordID,
            extractExpenseReport_result.data
          );
          log.debug({
            title: "getExpenseReport_result",
            details: getExpenseReport_result,
          });
      ...
      let extractJournalEntries_response;
      {
        ...
        extractJournalEntries_response = extractJournalEntries(
          recordSubsidiary,
          recordPeriod,
          recordID
        );
        log.debug({
          title: "extractJournalEntries_response",
          details: extractJournalEntries_response,
        });
      ...
      var dataReturn = {};
      {
        // agrupar información
        log.debug({
          title: "extractJournalEntries_response.data",
          details: extractJournalEntries_response.data,
        });
        log.audit({
          title: "getVendorBills_result DBG",
          details: getVendorBills_result,
        });
        // log.audit({title:'getExpenseReport_result.data',details:getExpenseReport_result.data});
        // dataReturn = Object.assign(dataReturn, getVendorBills_result?getVendorBills_result.data:{}, getExpenseReport_result ? getExpenseReport_result.data : {});
        dataReturn = Object.assign(
          dataReturn,
          getVendorBills_result ? getVendorBills_result.data : {},
          getExpenseReport_result ? getExpenseReport_result.data : {},
          extractJournalEntries_response
            ? extractJournalEntries_response.data
            : {}
        );
      ...
      var otherId = record.submitFields({
        ...
        values: {
          [RECORD_INFO.DIOT_RECORD.FIELDS.STATUS]:
            STATUS_LIST_DIOT.VALIDATING_DATA,
  ...
  const map = (mapContext) => {
    try {
      ...
      log.debug({ title: "SUITETAX EN MAP", details: suitetax });
      ...
      if (!datos.hasOwnProperty("isJournalEntry")) {
        if (!datos.hasOwnProperty("isExpenseReport")) {
          log.emergency({
            title: "datos.transaccionInternalId DBG",
        ...
        let getApllyPayments_result = getApllyPayments(
          datos.transaccionInternalId,
          datos.vendorId,
          datos.diotRecord,
          datos.subsidiaria,
          datos.periodo
  ...
  const reduce = (reduceContext) => {
    ...
    try {
      ...
      log.debug({
        ...
        details: STATUS_LIST_DIOT.BUILDING,
      });
      var otherId = record.submitFields({
        type: RECORD_INFO.DIOT_RECORD.ID,
        id: recordID,
    ...
    log.audit({
      ...
      var salestaxitemSearchObj = search.create({
        ...
        columns: [
          ...
          search.createColumn({
            name: "custrecord_fb_diot_tipo_imp",
            label: "DIOT - Tipo de impuesto",
          }),
        ],
      });
      ...
      var codigosReporte = {
        [RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.IVA]: [],
        [RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.RETENCION]: [],
        [RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.IEPS]: [],
        [RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.EXENTO]: [],
      };
      ...
      if (searchTaxCodesNS.count > 0) {
        searchTaxCodesNS.pageRanges.forEach(function (pageRange) {
          ...
          myPage.data.forEach(function (result) {
            let codigoTipo = result.getValue({
              name: "custrecord_fb_diot_tipo_imp",
            });
            let codigoID = result.getValue({ name: "internalid" });
            let codigoNombre = result.getValue({ name: "name" });
            codigosReporteIds.push(codigoID);
            log.audit({ title: "codigoTipo ❇️❇️❇️", details: codigoTipo });
            switch (codigoTipo) {
              case "1": // IVA
                codigosReporte[RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.IVA].push(
                  { value: codigoID, text: codigoNombre }
                );
                break;
              case "2": // IEPS
                codigosReporte[
                  RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.IEPS
                ].push({ value: codigoID, text: codigoNombre });
                break;
              case "3": // retenciones
                codigosReporte[
                  RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.RETENCION
        ...
        // si no hay impuestos configurados
        throw generateError(
          "ERROR DE PROCESAMIENTO",
          "No se encontraron Impuestos configurados para reportar.",
          "Error, no hay codigos de impuestos configurados para el reporte DIOT."
        );
      }
      if (suitetax) {
        log.audit({ title: "Suitetax es TRUE, se usa IVAS RESPONSE" });
        ...
        if (codigosReporte[RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.IVA].length) {
          ivas_response = extractTaxes(
            codigosReporte[RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.IVA],
            values
          );
        ...
        if (
          codigosReporte[RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.RETENCION]
            .length
        ) {
          retenciones_response = extractTaxes(
            codigosReporte[RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.RETENCION],
            values
          );
        ...
        if (
          codigosReporte[RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.IEPS].length
        ) {
          ieps_respose = extractTaxes(
            codigosReporte[RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.IEPS],
            values
          );
        ...
        if (
          codigosReporte[RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.EXENTO].length
        ) {
          exento_response = extractTaxes(
            codigosReporte[RECORD_INFO.DESGLOSE_TAX_RECORD.FIELDS.EXENTO],
            values
          );
      ...
      let diotLine =
        generalInfo.vendorTipoTercero_Code +
        "|" +
        generalInfo.transaccionTipoOperacion_Code +
        "|" +
        generalInfo.vendorRFC +
        "|" +
        (generalInfo.vendorTaxId != null ? generalInfo.vendorTaxId : "") +
        "|" +
        generalInfo.vendorNombreExtranjero +
        "|" +
        generalInfo.vendorPaisResidencia_Code +
        "|" +
        generalInfo.vendorNacionalidad +
        "|";

      if (suitetax) {
        if (Object.keys(ivas_response.dataClear).length > 0) {
          log.debug({
            title: "ivas_response.dataClear DB",
            details: Object.keys(ivas_response.dataClear).length > 0,
          });
          if (
            ivas_response.dataClear["16.0%"] &&
            generalInfo.vendorTipoTercero == LISTS.TIPO_TERCERO.VALUES.NACIONAL
          ) {
            diotLine += Math.ceil(ivas_response.dataClear["16.0%"].total);
            sumatorias["IVA al 16%"] += Math.ceil(
              ivas_response.dataClear["16.0%"].total
            );
          }
          diotLine += "|||||";
          if (ivas_response.dataClear["8.0%"]) {
            diotLine += Math.ceil(ivas_response.dataClear["8.0%"].total);
            sumatorias["IVA al 8%"] += Math.ceil(
              ivas_response.dataClear["8.0%"].total
            );
          }
          diotLine += "|||";
          if (
            ivas_response.dataClear["16.0%"] &&
            generalInfo.vendorTipoTercero ==
              LISTS.TIPO_TERCERO.VALUES.EXTRANJERO
          ) {
            diotLine += Math.ceil(ivas_response.dataClear["16.0%"].total);
          }
          diotLine += "||||";
          if (
            ivas_response.dataClear["0.0%"] &&
            generalInfo.vendorTipoTercero ==
              LISTS.TIPO_TERCERO.VALUES.EXTRANJERO
          ) {
            diotLine += Math.ceil(ivas_response.dataClear["0.0%"].total);
            sumatorias["IVA al 0%"] += Math.ceil(
              ivas_response.dataClear["0.0%"].total
            );
          }
          diotLine += "|";
          if (
          ...
          if (exento_response) {
            if (
              Object.keys(exento_response.dataClear).length > 0 &&
              generalInfo.vendorTipoTercero ==
                LISTS.TIPO_TERCERO.VALUES.NACIONAL
            ) {
              // columna 22
              // log.debug({ title:'exento_response.dataClear', details:exento_response });
              diotLine += Math.ceil(exento_response.dataClear["0.0%"].total);
              sumatorias["Exentos"] += Math.ceil(
                exento_response.dataClear["0.0%"].total
              );
          ...
          if (retenciones_response) {
            if (Object.keys(retenciones_response.dataClear).length > 0) {
              ...
              if (
                suma_impuestos_response.success == true &&
                suma_impuestos_response.totalImpuestos
              ) {
          ...
          let extract_devoluciones_response = extract_devoluciones(
            generalInfo.subsidiaria,
            generalInfo.periodo,
            generalInfo.vendorId,
            generalInfo.transaccionTipoOperacion,
            codigosReporteIds,
            generalInfo.vendorRFC
          );
          log.debug({
            title: "DEVOLUCIONES Creditos",
            details: extract_devoluciones_response,
          });
          if (extract_devoluciones_response.success == true) {
            if (extract_devoluciones_response.totalDevoluciones) {
              devolucionesImporte =
                devolucionesImporte +
                extract_devoluciones_response.totalDevoluciones * 1;
            }
          } else {
          ...
          log.debug({
            title: "DEVOLUCIONES JE",
            details: getJEDevoluciones_response,
          });
          if (getJEDevoluciones_response.success == true) {
            if (getJEDevoluciones_response.total != 0) {
              devolucionesImporte =
                devolucionesImporte * 1 + getJEDevoluciones_response.total * 1;
            }
          } else {
            throw getJEDevoluciones_response.error;
          }
          if (devolucionesImporte != 0) {
            diotLine += Math.ceil(devolucionesImporte);
            sumatorias["Importe Devoluciones"] +=
              Math.ceil(devolucionesImporte);
          }
        }
      }
      ...
      if (!suitetax) {
        ...
        const recordPeriod =
          registerData[RECORD_INFO.DIOT_RECORD.FIELDS.PERIOD][0].value;
        let resultado = getPaymentsPeriod(recordSubsidiary, recordPeriod);
        let transacciones = extractVendorBill(resultado);
        log.debug({ title: "TRANSACCIONES", details: transacciones });
          ...
          vendorbillResult.pageRanges.forEach(function (pageRange) {
            ...
            myPage.data.forEach(function (result) {
              if (
                ...
                // Obtener el texto del campo taxJson
                ...
                log.debug({
                  title: "Error",
                  details: "El campo taxJson no está definido en el resultado",
                });
              }
            });
          });
        ...
        diotLine += "|";
        if (
          parseFloat(taxJsonObject.exento_total)!=0 &&
          generalInfo.vendorTipoTercero == LISTS.TIPO_TERCERO.VALUES.NACIONAL
        ) {
          log.debug({title:"exento total", details:parseFloat(taxJsonObject.exento_total)})
          diotLine += Math.ceil(parseFloat(taxJsonObject.exento_total));
          sumatorias["Exentos"] += Math.ceil(
            parseFloat(taxJsonObject.exento_total)
          );
        }
        diotLine += "|";
        log.debug({title:"RETENCION TOTAL", details:parseFloat(taxJsonObject.retencion_total)})
        if (parseFloat(taxJsonObject.retencion_total) != 0) {
          // if (suma_impuestos_response.success == true && suma_impuestos_response.totalRetenciones) {
          diotLine += Math.ceil(parseFloat(taxJsonObject.retencion_total));
          // diotLine += suma_impuestos_response.totalRetenciones;
        }
        diotLine += "|";
        let devolucionesImporte = 0;
        ...
        if (devolucionesImporte != 0) {
          diotLine += Math.ceil(devolucionesImporte);
          sumatorias["Importe Devoluciones"] += Math.ceil(devolucionesImporte);
        }
      }
      log.audit({ title: "Fuera del if(suitetax==false)" });
      diotLine += "|\n";
      // Función para formatear números con comas para separar los miles y los millones
      function formatNumberWithCommas(number) {
        return number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
      }
      // Modificar el objeto sumatorias para formatear los números con comas y el signo de dólar
      const formattedSumatorias = {
      ...
      var summatoryRecord = record.submitFields({
        type: RECORD_INFO.DIOT_RECORD.ID,
        id: recordID,
        values: {
          [RECORD_INFO.DIOT_RECORD.FIELDS.SUMATORY]: formattedSumatorias,
        },
      });

      // summatoryRecord.setValue({
      //     fieldId:  RECORD_INFO.DIOT_RECORD.FIELDS.SUMATORY ,
      //     value:sumatorias
      // });
   ...
   * @param {Object} summaryContext.reduceSummary - Statistics about the reduce stageobjLine
   * @since 2015.2
   */
  const summarize = (summaryContext) => {
    ...
    const notificar = objScript.getParameter({
      name: SCRIPTS_INFO.MAP_REDUCE.PARAMETERS.NOTIFICAR,
    });
    try {
      ...
      const folderRaiz = objScript.getParameter({
        name: SCRIPTS_INFO.MAP_REDUCE.PARAMETERS.FOLDER_RAIZ,
      });
      var hasErrors = false;
      {
        ...
        // log.audit({title: 'Estado_getInput', details: registerData});
        var recordSubsidiary =
          registerData[RECORD_INFO.DIOT_RECORD.FIELDS.SUBSIDIARY][0].text;
        var recordPeriod =
          registerData[RECORD_INFO.DIOT_RECORD.FIELDS.PERIOD][0].text;
        ...
        let validateFolder_response = validateFolder(
          folderRaiz,
          recordSubsidiary,
          recordPeriod
        );
        if (
          validateFolder_response.success == false ||
          !validateFolder_response.folderID
        ) {
      ...
      }
      {
        ...
        txtNombre =
          "DIOT_" + recordSubsidiary + "_" + recordPeriod + "_" + dateStructure;
      }
      {
        ...
        output.iterator().each((key, value) => {
          ...
          log.debug({
            title: "salida DBG",
            details: { key: key, value: jsonValue },
          });
            ...
            if (txtDiot === "" && hasErrors_inSomeTrans == false) {
              ...
              let otherId = record.submitFields({
                ...
                values: {
                  [RECORD_INFO.DIOT_RECORD.FIELDS.FILE]: fileId,
                  [RECORD_INFO.DIOT_RECORD.FIELDS.STATUS]:
                    STATUS_LIST_DIOT.PARTIALLY_COMPLETE,
                  [RECORD_INFO.DIOT_RECORD.FIELDS.MESSAGE]:
                    "Favor de atender los errores presentados y volver a generar para incluirlos dentro de su archivo DIOT.",
                  [RECORD_INFO.DIOT_RECORD.FIELDS.PROGRESS]: 100,
                },
              });
              log.audit({
                title: "otherId PARTIAL COMPLETE STATUS",
                details: otherId,
              });
            } else {
              let otherId = record.submitFields({
                ...
                values: {
                  [RECORD_INFO.DIOT_RECORD.FIELDS.FILE]: fileId,
                  [RECORD_INFO.DIOT_RECORD.FIELDS.STATUS]:
                    STATUS_LIST_DIOT.COMPLETE,
                  [RECORD_INFO.DIOT_RECORD.FIELDS.MESSAGE]:
                    "Archivo DIOT creado con éxito.",
  ...
  const generateRecordError = (
    ...
    try {
      ...
      log.audit({
        title: "RECORD_INFO.ERRORES_DIOT.FIELDS DBG",
        details: detail,
      });
    ...
    try {
      var vendorbillSearchObj = search.create({
        ...
        columns: [
          ...
          search.createColumn({
            name: "custbody_fb_tipo_operacion",
      ...
      if (vendorbillResult.count > 0) {
        ...
        vendorbillResult.pageRanges.forEach(function (pageRange) {
          var myPage = vendorbillResult.fetch({ index: pageRange.index });
          myPage.data.forEach(function (result) {
            let vendorTipoTercero = result.getValue({
              name: RECORD_INFO.VENDOR_RECORD.FIELDS.TIPO_TERCERO,
              join: RECORD_INFO.VENDOR_RECORD.ID,
            });
            let vendorId = result.getValue({
              name: RECORD_INFO.VENDOR_RECORD.FIELDS.INTERNALID,
              join: RECORD_INFO.VENDOR_RECORD.ID,
            });
            let vendorTipoTercero_Text = result.getText({
              name: RECORD_INFO.VENDOR_RECORD.FIELDS.TIPO_TERCERO,
              join: RECORD_INFO.VENDOR_RECORD.ID,
            });
            let vendorTipoTercero_Code = vendorTipoTercero_Text.split(" ");
            vendorTipoTercero_Code = vendorTipoTercero_Code[0];
            let vendorbillTipoOperacion = result.getValue({
              name: RECORD_INFO.VENDOR_BILL_RECORD.FIELDS.TIPO_OPERACION,
            });
            let vendorbillTipoOperacion_Text = result.getText({
              name: RECORD_INFO.VENDOR_BILL_RECORD.FIELDS.TIPO_OPERACION,
            });

            log.debug({
              title: "TipoOperacion_Text ",
              details: vendorbillTipoOperacion_Text,
            });

            let vendorbillTipoOperacion_Code =
              vendorbillTipoOperacion_Text.split(" ");
            vendorbillTipoOperacion_Code = vendorbillTipoOperacion_Code[0];
            let vendorbillTranId = result.getValue({
              name: RECORD_INFO.VENDOR_BILL_RECORD.FIELDS.TRANID,
            });
            let vendorbillInternalId = result.getValue({
              name: RECORD_INFO.VENDOR_BILL_RECORD.FIELDS.ID,
            });
            if (vendorTipoTercero) {
              if (vendorbillTipoOperacion) {
                let vendorRFC = result.getValue({
                  name: RECORD_INFO.VENDOR_RECORD.FIELDS.RFC,
                  join: RECORD_INFO.VENDOR_RECORD.ID,
                });
                let vendorNombreExtranjero = result.getValue({
                  name: RECORD_INFO.VENDOR_RECORD.FIELDS.NOMBRE_EXTRANJERO,
                  join: RECORD_INFO.VENDOR_RECORD.ID,
                });
                let vendorPaisResidencia = result.getValue({
                  name: RECORD_INFO.VENDOR_RECORD.FIELDS.PAIS_RESIDENCIA,
                  join: RECORD_INFO.VENDOR_RECORD.ID,
                });
                let vendorPaisResidencia_Text = result.getText({
                  name: RECORD_INFO.VENDOR_RECORD.FIELDS.PAIS_RESIDENCIA,
                  join: RECORD_INFO.VENDOR_RECORD.ID,
                });
                let vendorPaisResidencia_Code =
                  vendorPaisResidencia_Text.split(" ");
                vendorPaisResidencia_Code = vendorPaisResidencia_Code[0];
                let vendorNacionalidad = result.getValue({
                  name: RECORD_INFO.VENDOR_RECORD.FIELDS.NACIONALIDAD,
                  join: RECORD_INFO.VENDOR_RECORD.ID,
                });
                let vendorTaxId = result.getValue({
                  name: RECORD_INFO.VENDOR_RECORD.FIELDS.TAX_ID,
                  join: RECORD_INFO.VENDOR_RECORD.ID,
                });
                  ...
                  case LISTS.TIPO_TERCERO.VALUES.NACIONAL:
                    if (!vendorRFC) {
                      let errorObj = {
                        ...
                        cause:
                          "Error, No se tiene configurado el rfc del proveedor.",
                        proveedor: vendorId,
                        transaccion: "",
                      };
                      let errorFound = errorsTrans.findIndex(
                        (element) =>
                          element.code == errorObj.code &&
                          element.msg == errorObj.msg &&
                          element.proveedor == errorObj.proveedor
                      );
                      if (errorFound == -1) {
                  ...
                  case LISTS.TIPO_TERCERO.VALUES.EXTRANJERO:
                    ...
                    if (
                      ...
                      let errorObj = {
                        ...
                        cause:
                          "Error, No se tiene configurado el proveedor extranjero.",
                        proveedor: vendorId,
                        transaccion: "",
                      };
                      let errorFound = errorsTrans.findIndex(
                  ...
                  case LISTS.TIPO_TERCERO.VALUES.GLOBAL:
                    if (!vendorRFC) {
                      ...
                      let errorObj = {
                        ...
                        msg: "No hay rfc para el proveedor",
                        cause:
                          "Error, No se tiene configurado el proveedor global.",
                        proveedor: vendorId,
                        transaccion: "",
                      };
                      let errorFound = errorsTrans.findIndex(
                ...
                let errorObj = {
                  ...
                  cause:
                    "Error, No se tiene configurado el tipo de operacion en la transaccion.",
                  proveedor: vendorId,
                ...
                let errorFound = errorsTrans.findIndex(
                  (element) =>
                    ...
                    element.msg == errorObj.msg &&
                    element.proveedor == errorObj.proveedor &&
                    element.transaccion == errorObj.transaccion
                );
                if (errorFound == -1) {
                  errorsTrans.push(errorObj);
                }
              ...
              let errorObj = {
                ...
                cause:
                  "Error, No se tiene configurado el tipo de tercero del proveedor.",
                proveedor: vendorId,
                transaccion: "",
              };
              let errorFound = errorsTrans.findIndex(
        ...
        vendorbillFound.forEach((element, index) => {
          ...
          let identify =
            element.transaccionTranId + "_" + element.transaccionInternalId;
        ...
        let newError = generateError(
          "ERROR DE PROCESAMIENTO",
          "No se encontraron Facturas a reportar con los datos ingresados.",
          "Error, la busqueda guardada regreso una cantidad de resultados cero."
        );
        generateRecordError(
          "ERROR DE PROCESAMIENTO",
          "No se encontraron Facturas a reportar con los datos ingresados.",
   ...
   */
  const getApllyPayments = (
    vendorbillInternalId,
    vendorId,
    diotRecord,
    subsidiaria,
    periodo
      ...
      var vendorpaymentSearchObj = search.create({
        ...
        columns: [
          ...
          search.createColumn({
            ...
            label: "Aplicado al vínculo importe",
          }),
        ],
      });
      ...
      if (vendorbillResult.count > 0) {
        vendorbillResult.pageRanges.forEach(function (pageRange) {
          ...
          myPage.data.forEach(function (result) {
            ...
            let transaccionId = result.getValue({
              name: "internalid",
              join: "appliedToTransaction",
            });
            let transaccionName = result.getValue({
              name: "transactionname",
              join: "appliedToTransaction",
            });
            ...
            paymentAmountApply =
              paymentAmountApply +
              Number(result.getValue({ name: "appliedtolinkamount" }));
            ...
            if (!payments.hasOwnProperty(identify)) {
              ...
              payments[identify].paymentAmountApply =
                payments[identify].paymentAmountApply * 1 +
                paymentAmountApply * 1;
            }
          });
        ...
        let newError = generateError(
          "ERROR NO PAGOS",
          "La factura no cuenta con pagos registrados",
          "La factura id: " + vendorbillInternalId + " no contiene pagos."
        );
        generateRecordError(
          "ERROR NO PAGOS",
          "La factura no cuenta con pagos registrados",
          ...
          taxes.forEach((tax, index_tax) => {
            ...
            if (isDevolucion == false) {
              if (tax.taxCode == element_impuesto.value) {
                ...
                if (
                  (factura.transaccionEstado != "open" &&
                    !factura.hasOwnProperty("isExpenseReport")) ||
                  (factura.transaccionEstado != "approvedByAcct" &&
                    factura.hasOwnProperty("isExpenseReport")) ||
                  (factura.transaccionEstado != "approvedByAcct" &&
                  ...
                  log.audit({ title: "totalpagado 🪼", details: totalpagado });
                  log.audit({
                    title: "tax.taxBasis 🐧",
                    details: tax.taxBasis,
                  });
                  log.audit({
                    title: "factura.transaccionImporte ❤️",
                    details: factura.transaccionImporte,
                  });
                  let equivalentePagado =
                    (totalpagado * 1 * (tax.taxBasis * 1)) /
                    (factura.transaccionImporte * 1);
  ...
  const validateFolder = (folderId, subsidiaria, periodo) => {
    ...
    try {
      ...
      if (subsidiariafolder_result.folderID == -1) {
        //crear folder
      ...
      let periodoFolder;
      if (periodoFolder_result.folderID == -1) {
        // crear folder
        ...
        myPagedData.pageRanges.forEach(function (pageRange) {
          ...
          myPage.data.forEach(function (result) {
            ...
            if (taxCodes.indexOf(taxCode) != -1) {
              if (suitetax) {
                let taxBasis = result.getValue({
                  name: "taxbasis",
                  join: "taxDetail",
                });
                response.totalDevoluciones =
                  response.totalDevoluciones * 1 + taxBasis * 1;
              } else {
                let taxBasis;
                if (taxJsonObject.bases_iva["16"])
                  taxBasis = taxJsonObject.bases_iva["16"];
                ...
                if (taxJsonObject.bases_iva["0"])
                  taxBasis = taxJsonObject.bases_iva["0"];
                response.totalDevoluciones =
                  response.totalDevoluciones.toFixed(2) + taxBasis * 1;
              }
            }
          });
        });
  ...
  function send_email_status(recordId) {
    ...
    try {
      ...
      email.send({
        ...
        body:
          "Estimado usuario el proceso de generación del reporte DIOT ha concluido, revise su registro de seguimiento con id: " +
          recordId,
      });
    } catch (error) {
      response.success = false;
      ...
      if (suitetax) {
        var expensereportSearchObj = search.create({
          ...
          columns: [
            ...
            search.createColumn({
              name: "custcol_fb_proveedor",
              label: "Proveedor",
            }),
            search.createColumn({
              name: "custcol_fb_diot_prov_type",
              label: " Tipo de tercero",
            }),
            search.createColumn({
              name: "custcol_fb_diot_rfc_proveedot",
              label: "RFC Proveedor",
            }),
            search.createColumn({
              name: "custbody_fb_tipo_operacion",
            ...
            search.createColumn({
              name: "total",
              label: "Importe (total de transacción)",
            }),
          ],
        });
      } else {
      ...
      if (expenseReportResult.count > 0) {
        ...
        expenseReportResult.pageRanges.forEach(function (pageRange) {
          ...
          myPage.data.forEach(function (result) {
            ...
            let vendorTipoTercero =
              result.getValue({ name: "custcol_fb_diot_prov_type" }) || 1;
            let vendorTipoTercero_Text =
              result.getText({ name: "custcol_fb_diot_prov_type" }) ||
              "04 - Proveedor Nacional";
            let vendorTipoTercero_Code = vendorTipoTercero_Text.split(" ");
            vendorTipoTercero_Code = vendorTipoTercero_Code[0];
            let expenseReportTranId = result.getValue({ name: "tranid" });
            let expenseReportInternalId = result.getValue({
              name: "internalid",
            });
            let expenseReportEstado = result.getValue({ name: "statusref" });
            let expenseReportImporte = result.getValue({ name: "total" });
            let expenseReportTipoOperacion =
              result.getValue({ name: "custbody_fb_tipo_operacion" }) || 3;
            let expenseReportTipoOperacion_Text =
              result.getText({ name: "custbody_fb_tipo_operacion" }) ||
              "85 - Otros";
            let expenseReportTipoOperacion_Code =
              expenseReportTipoOperacion_Text.split(" ");
            expenseReportTipoOperacion_Code =
              expenseReportTipoOperacion_Code[0];
            if (suitetax) {
              let taxBasis = result.getValue({
                name: "taxbasis",
                join: "taxDetail",
              ...
              let taxType = result.getValue({
                name: "taxtype",
                join: "taxDetail",
              });
              let taxAmount = result.getValue({
                name: "taxamount",
                join: "taxDetail",
              });
              let taxRate = result.getValue({
                name: "taxrate",
                join: "taxDetail",
              });
              let objTax = {
            ...
            let expenseFound = expenseReportArray.findIndex(
              (element) =>
                element.vendorRFC == vendorRFC &&
                element.transaccionTipoOperacion == expenseReportTipoOperacion
            );
            if (expenseFound == -1) {
        ...
        expenseReportArray.forEach((element, index) => {
          ...
          let identify =
            "Expense" +
            "_" +
            element.transaccionInternalId +
            "_" +
            element.vendorRFC +
            "_" +
            element.transaccionTipoOperacion;
      ...
      var vendorpaymentSearchObj = search.create({
        ...
        columns: [
          ...
          search.createColumn({
            ...
            label: "Aplicado a la transacción",
          }),
          search.createColumn({
            name: "type",
            join: "appliedToTransaction",
      ...
      if (vendorbillResult.count > 0) {
        ...
        vendorbillResult.pageRanges.forEach(function (pageRange) {
          ...
          myPage.data.forEach(function (result) {
            ...
            let transactionApply = result.getValue({
              name: "appliedtotransaction",
            });
            let typeTransactionApply = result.getValue({
              name: "type",
              join: "appliedToTransaction",
            });
            ...
            if (
              typeTransactionApply == "VendBill" ||
              typeTransactionApply == "ExpRept"
            ) {
              ...
              if (transactionApply != "") {
                ...
                if (allPayments.length > 0) {
                  let indexFound = allPayments.findIndex(
                    (element) => element == prefijo
                  );
                  if (indexFound != -1) {
                    // se encontro
                    // if (transaccionIndex == -1) {
                    if (typeTransactionApply == "VendBill") {
                      let transaccionIndex = paymentsFound[prefijo][
                        "transacciones"
                      ]["vendBill"].findIndex(
                        (element) => element == transactionApply
                      );
                      if (transaccionIndex == -1) {
                        paymentsFound[prefijo]["transacciones"][
                          "vendBill"
                        ].push(transactionApply);
                      }
                    ...
                    if (typeTransactionApply == "ExpRept") {
                      ...
                      if (transaccionIndex == -1) {
                        paymentsFound[prefijo]["transacciones"]["expRept"].push(
                          transactionApply
                        );
                      }
                    ...
                    if (typeTransactionApply == "VendBill") {
                      paymentsFound[prefijo] = {
                        pagoid: internalId,
                        transacciones: {
                          vendBill: [transactionApply],
                          expRept: [],
                        },
                      };
                    }
  ...
  function extractJournalEntries(subsidiaria, periodo, recordID) {
    ...
    try {
      log.debug({
        title: "extractJournalEntries_params",
        details: {
          subsidiaria: subsidiaria,
          periodo: periodo,
          recordID: recordID,
      ...
      if (suitetax) {
        var journalentrySearchObj = search.create({
          ...
          columns: [
            ...
            search.createColumn({
              name: "custcol_fb_diot_operation_type",
              label: "Tipo de operación",
            }),
      ...
      log.debug({
        title: "journalEntriesResult.count",
        details: journalEntriesResult.count,
      });
      if (journalEntriesResult.count > 0) {
        ...
        journalEntriesResult.pageRanges.forEach(function (pageRange) {
          ...
          myPage.data.forEach(function (result) {
            ...
            let vendorId =
              result.getValue({ name: "custcol_fb_proveedor" }) || "";
            let vendorRFC =
              result.getValue({ name: "custcol_fb_diot_rfc_proveedot" }) || "";
            let vendorTipoTercero = result.getValue({
              name: "custcol_fb_diot_prov_type",
            });
            let vendorTipoTercero_text =
            ...
            let vendorTipoTercero_Code = vendorTipoTercero_text.split(" ");
            vendorTipoTercero_Code = vendorTipoTercero_Code[0];
            let jeTipoOperacion = result.getValue({
              name: "custcol_fb_diot_operation_type",
            });
            let jeTipoOperacion_text =
              result.getText({ name: "custcol_fb_diot_operation_type" }) ||
              "85 - Otros";
            ...
            if (suitetax) {
              ...
              var taxJsonObject = JSON.parse(taxJson);
              let taxAmount = taxJsonObject.iva_total;
            ...
            let jeFound = journalEntriesFound.findIndex(
              (element) =>
                element.vendorRFC == vendorRFC &&
                element.transaccionTipoOperacion == jeTipoOperacion
            );
            if (jeFound == -1) {
              let jeObj = {
                diotRecord: recordID,
        ...
        log.debug({
          title: "journalEntriesFound",
          details: journalEntriesFound,
        });
        var jeFoundClear = {};
